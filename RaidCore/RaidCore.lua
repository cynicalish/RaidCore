...
function RaidCore:OnRaidCoreOn(cmd, args)
    ...
    elseif (tAllParams[1] == "summon") then
       	self:SyncSummon()
	elseif (tAllParams[1] == "togglesummon") then
		if bAcceptSummons then
			bAcceptSummons = false
		else
			bAcceptSummons = true
		end
...
function RaidCore:OnComMessage(channel, strMessage, strSender)
    ...
    elseif tMessage.action == "SyncSummon" then
		if not self.settings["General"]["bAcceptSummons"] then
			return false
        elseif not self.settings["General"]["bAcceptSummons"] or not self:isRaidManagement(strSender) then
            return false
        end
